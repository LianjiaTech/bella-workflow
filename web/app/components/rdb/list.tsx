...
const WorkflowAppLogList: FC<ILogs> = ({ list, onRefresh, disabled }) => {
  const { t } = useTranslation()
  const { notify } = useContext(ToastContext)
  const [currentItem, setCurrentItem] = useState<object | undefined>()
  ...
  const toggleDs = async (datasourceId: string, status: number) => {
    if (status === 0) {
      await deactiveDatasource({ datasourceId, type: 'rdb' })
      notify({ type: 'success', message: t('datasource.table.toggle.deactivated') })
    }
    else {
      await activeDatasource({ datasourceId, type: 'rdb' })
      ...
      <table className={`w-full min-w-[440px] border-collapse border-0 text-sm mt-3 ${s.logTable}`}>
        <thead className="h-8 !pl-3 py-2 leading-[18px] border-b border-gray-200 text-xs text-gray-500 font-medium">
          <tr>
            ...
            <td className='whitespace-nowrap'>{t('datasource.table.dbType')}</td>
        ...
        <tbody className="text-gray-700 text-[13px]">
          {list.map((t: object) => {
            return <tr
              key={t?.datasourceId}
              className={`border-b border-gray-200 h-8 hover:bg-gray-50 cursor-pointer ${currentItem?.datasourceId !== t.datasourceId ? '' : 'bg-gray-50'}`}
              onClick={() => {
                setCurrentItem(t)
              }}>
              <td>{t.datasourceId}</td>
              <td>{t.dbType}</td>
              <td>{t.host}</td>
              <td>{t.port}</td>
              <td>{t.db}</td>
              <td>{t.user}</td>
              <td>{t.status === 0 ? 'active' : 'deactive'}</td>
              <td>
                <div className="flex items-center gap-2">
                  <Button disabled={disabled} variant="secondary" size="small" onClick={() => { toggleDs(t.datasourceId, t.status) }}>
                    {t.status === 0 ? <RiShutDownLine className="h-4 w-4" /> : <RiRestartLine className="h-4 w-4" />}
                  </Button>
                </div>
              </td>
            </tr>
