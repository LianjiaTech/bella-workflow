...
import { fetchWorkflowCustomApis } from '@/service/workflow'
import type { App, AppMode } from '@/types/app'
import CreateCustomApiModal from '@/app/components/app/create-custom-api-modal'
  ...
  return <div className='flex items-center justify-center h-full'>
    <div className='bg-gray-50 w-[560px] h-fit box-border px-5 py-4 rounded-2xl'>
      ...
      <div className='mt-2 text-gray-500 text-sm font-normal'>
        <Trans
          i18nKey="workflow.customApi.table.empty.element.content"
...
const WorkflowCApi: FC<ILogsProps> = ({ appDetail }) => {
  const { t } = useTranslation()
  ...
  }, fetchWorkflowCustomApis)

  const total = workflowLogs?.data.length || 0

  return (
    <div className='flex flex-col h-full'>
      <h1 className='text-md font-semibold text-gray-900'>{t('workflow.customApi.title')}</h1>
      <p className='flex text-sm font-normal text-gray-500'>{t('workflow.customApi.desc')}</p>
      <div className='flex flex-col py-4 flex-1'>
        <div className='flex flex-row justify-between'>
          <Button variant='primary' onClick={addTrigger}>{t('workflow.customApi.add')}</Button>
        </div>
        {/* workflow log */}
