      ...
      requiredVars.forEach(({ variable, label, type }) => {
        ...
        if ((type === InputVarType.singleFile || type === InputVarType.multiFiles) && inputs[variable]) {
          ...
          if (Array.isArray(files))
            fileIsUploading = files.find(item => item.transferMethod === TransferMethod.local_file && !item.id)
          else
            fileIsUploading = files.transferMethod === TransferMethod.local_file && !files.id
        }
      })
    }
