...
const Redis: FC<ILogsProps> = ({ disabled = false }) => {
  const { t } = useTranslation()
  const [showCreateModal, setShowCreateModal] = useState(false)
  const { data: redisInstances, mutate, isLoading } = useDatasourceList('redis')

  const addDatasource = () => {
    setShowCreateModal(true)
  }

  const total = redisInstances?.length || 0
    ...
    <div className='flex flex-col h-full px-14 pt-2'>
      <div className='flex flex-col py-4 flex-1'>
        ...
        {isLoading
          ? <Loading type='app' />
          : total > 0
            ? <List disabled={disabled} list={redisInstances} onRefresh={mutate} />
            : <EmptyElement />
        }
      </div>
