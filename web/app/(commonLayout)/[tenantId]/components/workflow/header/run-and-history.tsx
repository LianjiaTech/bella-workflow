import type { FC } from 'react'
import { memo } from 'react'
import { useTranslation } from 'react-i18next'
import { RiLoader2Line, RiPlayLargeLine } from '@remixicon/react'
import { useStore } from '@/app/components/workflow/store'
import {
  useIsChatMode,
  useNodesReadOnly,
  useWorkflowStartRun,
} from '@/app/components/workflow/hooks'
import { WorkflowRunningStatus } from '@/app/components/workflow/types'
import ViewHistory from '@/app/components/workflow/header/view-history'
import Checklist from '@/app/components/workflow/header/checklist'
import cn from '@/utils/classnames'

type RunAndHistoryLabels = {
  run?: string
  running?: string
  debugAndPreview?: string
}

type RunAndHistoryProps = {
  labels?: RunAndHistoryLabels
}

const RunMode = memo(({ labels }: { labels?: RunAndHistoryLabels }) => {
  const { t } = useTranslation()
  const { handleWorkflowStartRunInWorkflow } = useWorkflowStartRun()
  const workflowRunningData = useStore(s => s.workflowRunningData)
  const isRunning = workflowRunningData?.result.status === WorkflowRunningStatus.Running
  return (
    <>
      <div
        className={cn(
          'flex items-center px-2.5 h-7 rounded-md text-[13px] font-medium text-components-button-secondary-accent-text',
          'hover:bg-state-accent-hover cursor-pointer',
          isRunning && 'bg-state-accent-hover !cursor-not-allowed',
        )}
        onClick={() => handleWorkflowStartRunInWorkflow()}
      >
        {
          isRunning
            ? (
              <>
                <RiLoader2Line className='mr-1 w-4 h-4 animate-spin' />
                {labels?.running ?? t('workflow.common.running')}
              </>
            )
            : (
              <>
                <RiPlayLargeLine className='mr-1 w-4 h-4' />
                {labels?.run ?? t('workflow.common.run')}
              </>
            )
        }
      </div>
    </>
  )
})
RunMode.displayName = 'RunMode'

const PreviewMode = memo(({ labels }: { labels?: RunAndHistoryLabels }) => {
  const { t } = useTranslation()
  const { handleWorkflowStartRunInChatflow } = useWorkflowStartRun()

  return (
    <div
      className={cn(
        'flex items-center px-2.5 h-7 rounded-md text-[13px] font-medium text-components-button-secondary-accent-text',
        'hover:bg-state-accent-hover cursor-pointer',
      )}
      onClick={() => handleWorkflowStartRunInChatflow()}
    >
      <RiPlayLargeLine className='mr-1 w-4 h-4' />
      {labels?.debugAndPreview ?? t('workflow.common.debugAndPreview')}
    </div>
  )
})
PreviewMode.displayName = 'PreviewMode'
const RunAndHistory: FC<RunAndHistoryProps> = ({ labels }) => {
  const isChatMode = useIsChatMode()
  const { nodesReadOnly } = useNodesReadOnly()

  return (
    <div className='flex items-center px-0.5 h-8 rounded-lg border-[0.5px] border-components-button-secondary-border bg-components-button-secondary-bg shadow-xs'>
      {
        !isChatMode && <RunMode labels={labels} />
      }
      {
        isChatMode && <PreviewMode labels={labels} />
      }
      <div className='mx-0.5 w-[1px] h-3.5 bg-divider-regular'></div>
      <ViewHistory />
      <Checklist disabled={nodesReadOnly} />
    </div>
  )
}

export default memo(RunAndHistory)
