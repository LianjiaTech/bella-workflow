...
<Row>
    <Col>
        ...
        用户上传的文件ID集合 (<i>**注意**：所有文件必须在同一个Space下，Space概念请参考 [OpenAPI File-API](https://oq8cjme170.feishu.cn/docx/GeT0deG8eoM7BexxkJUc4yglnud)</i>)
        ...
        - B. 当`flashMode`为2时，A. + 工作流将不再保存输入的query、inputs、fileIds、metadata入参。
...
<Row>
    <Col>
        ...
        **重要提示：请求体中`file`字段除指定文件引用外，还必需指定Content-type (参考右侧curl)**

        **更具体、详细指导请参考：[OpenAPI File-API](https://oq8cjme170.feishu.cn/docx/GeT0deG8eoM7BexxkJUc4yglnud)**
        ...
        <Properties>
            ...
            <Property name='purpose' type='string' key='purpose'>
                上传文件的预期用途
            </Property>
        </Properties>

        ### Response
        成功上传后，服务器会返回文件的 ID 和相关信息。而后可将此ID用于Workflow Run中的fileIds参数。
        - `id` (uuid) 文件ID
        - `filename` (string) 文件名
        - `purpose` (string) 文件用途
        - `bytes` (long) 文件大小（byte）
        - `extension` (string) 文件后缀
        - `mime_type` (string) 文件 mime-type
        - `type` (string) 文件类型
        - `created_at` (timestamp) 上传时间（ms）
    </Col>
    <Col sticky>

        <CodeGroup title="Request" tag="POST" label="/files" targetCode={`curl -X POST '${props.appDetail.api_base_url}/files' \\\n--header 'Authorization: Bearer {api_key}' \\\n--form 'file=@localfile;type=image/[png|jpeg|jpg|webp|gif|...]' \\\n--form 'purpose={openapi_file_purpose}'`}>

            ```bash {{ title: 'cURL' }}
            curl -X POST '${props.appDetail.api_base_url}/files' \
            --header 'Authorization: Bearer {api_key}' \
            --form 'file={@localfile;type=image/[png|jpeg|jpg|webp|gif|...]}'
            --form 'purpose={openapi_file_purpose}'
            ```
        ...
        <CodeGroup title="Response">
            ```json {{ title: 'Response' }}
            {
                "id": "file-2502101517570026000353-2075695711",
                "object": "file",
                "bytes": 299567,
                "created_at": 1739171878000,
                "filename": "1.png",
                "purpose": "vision",
                "type": "image",
                "extension": "png",
