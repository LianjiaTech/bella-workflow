...
const KafkaList: FC<ILogs> = ({ list, onRefresh, disabled }) => {
  const { t } = useTranslation()
  const { notify } = useContext(ToastContext)
  const [currentItem, setCurrentItem] = useState<KafkaInstance | undefined>()
  ...
  const toggleDs = async (datasourceId: string, status: number) => {
    if (status === 0) {
      await deactiveDatasource({ datasourceId, type: 'kafka' })
      notify({ type: 'success', message: t('datasource.table.toggle.deactivated') })
    }
    else {
      await activeDatasource({ datasourceId, type: 'kafka' })
      ...
      <table className={`w-full min-w-[440px] border-collapse border-0 text-sm mt-3 ${s.logTable}`}>
        <thead className="h-8 !pl-3 py-2 leading-[18px] border-b border-gray-200 text-xs text-gray-500 font-medium">
          <tr>
            ...
            <td className='whitespace-nowrap'>{t('datasource.table.name')}</td>
            <td className='whitespace-nowrap'>{t('datasource.table.server')}</td>
            <td className='whitespace-nowrap'>{t('datasource.table.topic')}</td>
            <td className='whitespace-nowrap'>{t('datasource.table.type')}</td>
